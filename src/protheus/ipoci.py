...
class Oci:
    ...
    acceptedCommand = {
            'start': lambda region, iid: run(f'oci compute instance action --region {region} --instance-id {iid} --action START'),
            'stop': lambda region, iid: run(f'oci compute instance action --region {region} --instance-id {iid} --action SOFTSTOP'),
            'softstop': lambda region, iid: run(f'oci compute instance action --region {region} --instance-id {iid} --action SOFTSTOP'),
            'get': lambda region, iid: run(f'oci compute instance get --region {region} --instance-id {iid}')
        }
    ...
    def instance_oci(self, iid: str, region:str, action='get') -> bool:
        ...
        command = self.acceptedCommand[action.lower()]
        data = command(region, iid)
