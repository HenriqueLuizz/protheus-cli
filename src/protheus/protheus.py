...
@setup.command('config', short_help='Configura o diretório do broker, lista de exceção', help="Configura o diretório do broker, lista de conexão de  appserver slave que sempre fica ativo")
def config():
...
@setup.command('init', short_help='Inicializa ou zera o arquivo settings.json', help="Inicializa ou zer o arquivo settings.json que é responsável pelos parâmetros do PROTHEUS CLI")
def init():
    appdir=os.getcwd()
    ipset.init_setup()
...
@service.command('disable', short_help='Desabilita os serviços no broker protheus', help="Cria o arquivo .TOTVS_BROKER_COMMAND como todas as conexões de appserver configurado no appserver.ini do broker, exceto as conexões listada na chave ALWAYSDOWN no SETTINGS.JSON. \n\nTemplate do conteudo do arquivo: disable server 127.0.0.1:1234", epilog='')
def disable():
...
@service.command('list', short_help='Lista todos os serviços no BROKER PROTHEUS', help="Lista todos os serviços configurados no APPSERVER.INI do broker e as listas de exceções ALWAYSUP e ALWAYSDOWN do SETTINGS.JSON.", epilog='')
def list_service():
    ipserv.info(ipset)
...
@instance.command('stop', short_help='Desliga todas as instâncias. \n\n--quiet (quiet mode)', help="Desliga todas as instâncias configuradas no SETTINGS.JSON, envia um sinal de STOP para cada instância", epilog='')
@click.option('--quiet', '-q', is_flag=True, default=False, help='modo sem interação, o sinal de STOP será enviado sem solicitar confirmação')
def stop(quiet):
...
@instance.command('add', short_help='Adiciona um novo ID de instância, \n\n--iid <ID> (silent mode)', help="Adiciona um novo ID de instância no SETTINGS.JSON \n\n protheus instance add \n\n protheus instance add --iid <ID> \n\n protheus instance add --iid <ID> --iid <ID> ...", epilog='', deprecated=True)
...
@instance.command('remove', short_help='Remove um ID de instância, \n\n--iid <ID> (silent mode)', help="Remove um ID de instância no SETTINGS.JSON \n\n protheus instance remove \n\n protheus instance remove --iid <ID> \n\n protheus instance remove --iid <ID> --iid <ID> ...", epilog='', deprecated=True)
...
@sched.command('enableservice', short_help='Define um horário para o serviço ser habilitado no BROKER.', help="Define um horário para o serviço ser habilitado no BROKER. \n\n protheus sched enableservice \n\n protheus sched enableservice --hour <00:00>", epilog='')
...
@sched.command('disableservice', short_help='Define um horário para o serviço ser desabilitado no BROKER.', help="Define um horário para o serviço ser desabilitado no BROKER. \n\n protheus sched disableservice \n\n protheus sched disableservice --hour <00:00> (silent mode)", epilog='')
@click.option('--hour', '-h', multiple=False, help='Definir horário, formato: 00:00 (mode silent)')
...
@sched.command('startinstance', short_help='Define um horário para a instância ser ligada.', help="Define um horário para a instância ser ligada. \n\n protheus sched startinstance \n\n protheus sched startinstance --hour <00:00> (silent mode)", epilog='')
@click.option('--hour', '-h', multiple=False, help='Definir horário, formato: 00:00 (mode silent)')
def turnon(hour):
...
@sched.command('stopinstance', short_help='Define um horário para a instancia ser desligada', help="Define um horário para a instancia ser desligada. \n\n protheus sched stopinstance \n\n protheus sched stopinstance --hour <00:00> (silent mode)", epilog='')
@click.option('--hour', '-h', multiple=False, help='Definir horário, formato: 00:00 (mode silent)')
def turnoff(hour):
...
@sched.command('repeat', short_help='Define a recorrencia das execuções, por padrão é daily', help="Define a recorrencia de ligar e desligar as instâncias e habilitar e desabilitar os serviços. \n\n protheus sched repeat \n\n protheus sched repeat --rec <daily|weeky> (silent mode)", epilog='')
...
@sched.command('list', short_help='Lista as configurações do agendamento e os serviços alvo', help="Lista as configurações do agendamento de ligar e desligar as instâncias e habilitar e desabilitar os serviços.", epilog='')
def list_sched():
...
@sched.command('run', short_help='Inicia o processo de agendamento', help="Inicia o processo de agendamento, ao executar este comando este console ficará exclusivo para o agendamento. \n\nPara iniciar este processo como serviço verifique a documentação.", epilog='')
...
@update.command('rpo', short_help='Lista e atualiza os RPOs desatualizados do Protheus', help="Atualiza os artefatos do Protheus. \n\n protheus update rpo (lista os RPOs desatualizados)", epilog='')
