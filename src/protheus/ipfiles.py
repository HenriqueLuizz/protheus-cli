...
class Files:
    ...
    def need_update(self, auto_update=False, auto_create=False, force=False):
        ...
        if self.it_exists(path_master_full):
            ...
            log(f'Ultima modificação do {name} em {path_master} - ' + datetime.fromtimestamp(
                date_master).strftime('%d/%m/%Y %H:%M:%S'), 'INFO')
            ...
            for slv in path_slv:
                ...
                if os.path.exists(os.path.join(slv, name)):
                    ...
                    if date_slv < date_master or force:
                        log(f'{name} desatualizado em {slv} - ' + datetime.fromtimestamp(
                            date_slv).strftime('%d/%m/%Y %H:%M:%S'), 'INFO')
